...
export class BrickCityGame {
  ...
  constructor(
    ...
    this.scene.fog = new THREE.Fog('#bfe0f8', 120, 320)
  ...
  private buildCity() {
    ...
    const ground = new THREE.Mesh(new THREE.BoxGeometry(200, 2, 200), mat('#7cbf6b'))
    ground.position.y = -1
    g.add(ground)
    ...
    const treeSpots = [
      ...
      [-92, 60], [92, -60], [-55, -48], [55, -48], [0, -48], [0, 45], [92, 55],
    ]
  ...
  private update(dt: number) {
    ...
    if (this.mode === 'city' && this.doorCooldown <= 0) {
      ...
      if ((p.x - this.interiorExit.x) ** 2 + (p.z - this.interiorExit.z) ** 2 < 5) {
        this.exitBuilding()
      }
    }

    // camera: bird's-eye follow, or eye-level looking where the player faces
    if (this.view === 'first') {
      const rot = this.player.rotation.y
      const fx = Math.sin(rot)
      const fz = Math.cos(rot)
      const camTarget = new THREE.Vector3(p.x - fx * 1.2, 5.6, p.z - fz * 1.2)
      this.camera.position.lerp(camTarget, Math.min(1, dt * 10))
      this.camera.lookAt(p.x + fx * 12, 4.2, p.z + fz * 12)
    } else {
      const camTarget = this.mode === 'city'
